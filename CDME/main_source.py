...
if __name__ == "__main__":
    ...
    if args.dataset == "VisDA":
        ...
        args.source_data_dir = os.path.join("/root/autodl-tmp/BMD-main/data/DomainNet", names[args.s_idx])
        args.target_data_dir = os.path.join("/root/autodl-tmp/BMD-main/data/DomainNet", names[args.t_idx])
        args.class_num = 126
    ...
    if (not hasattr(args, "epochs")) or (args.epochs is None):
        args.epochs = 10  # 兜底
    if args.epochs == 10:  # 只在默认值为10时按数据集调整
        if args.dataset == "VisDA":
            args.epochs = 10
        else:  # OfficeHome / Office
            args.epochs = 50
